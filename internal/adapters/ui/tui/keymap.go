...
type keyMap struct {
	...
	ToggleURLs key.Binding
	ReplyAI    key.Binding
	Esc        key.Binding
	Enter      key.Binding
	// CycleFocus rotates sidebar → list → reader → sidebar.
...
func (k keyMap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		{k.Up, k.Down, k.Left, k.Right, k.CycleFocus},
...
func defaultKeyMap() keyMap {
	return keyMap{
		...
		ToggleURLs: key.NewBinding(key.WithKeys("U"), key.WithHelp("U", "full urls")),
		ReplyAI:    key.NewBinding(key.WithKeys("A"), key.WithHelp("A", "ai reply")),
		...
		CycleFocus: key.NewBinding(key.WithKeys("ctrl+h"), key.WithHelp("ctrl+h", "next pane")),
	}
}
